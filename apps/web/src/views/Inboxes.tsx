...
export function InboxesView({ orgId, currentUid }: { orgId: string; currentUid: string }) {
  ...
  const load = useCallback(async () => {
    const { data } = await supabase
      .from('inboxes')
      .select('id,name,inbound_address,from_domain,signature,dkim_verified,created_at')
      .eq('org_id', orgId)
      .order('created_at');
  ...
  async function remove(ib: Inbox) {
    if (!confirm(`受信箱「${ib.name}」を削除します。よろしいですか？（紐づくチケットがあると削除できません）`)) return;
      ...
      <section style={card}>
        ...
        {inboxes.length === 0 ? (
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
            {inboxes.map((ib) => (
              <li key={ib.id} style={{ padding: '12px 0', borderBottom: '1px solid #f0f0f0' }}>
